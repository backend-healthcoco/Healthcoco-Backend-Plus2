...
public interface NotesRepository extends MongoRepository<NotesCollection, ObjectId>, PagingAndSortingRepository<NotesCollection, ObjectId> {
    ...
    List<NotesCollection> findCustomGlobalNotes(ObjectId doctorId, ObjectId locationId, ObjectId hospitalId, Collection<String> specialities, Date date, boolean[] discards, Pageable pageable);
    ...
    List<NotesCollection> findCustomNotes(ObjectId doctorId, ObjectId locationId, ObjectId hospitalId, Date date, boolean[] discards, Pageable pageable);

    @Query("{'doctorId': ?0, 'updatedTime': {'$gt': ?1}, 'discarded': {$in: ?2}}")
    List<NotesCollection> findCustomNotes(ObjectId doctorId, Date date, boolean[] discards, Pageable pageable);
    ...
    List<NotesCollection> findCustomGlobalNotes(ObjectId doctorId, Collection<String> specialities, Date date, boolean[] discards, Pageable pageable);
    ...
    List<NotesCollection> findCustomGlobalNotes(ObjectId doctorId, ObjectId locationId, ObjectId hospitalId, Collection<String> specialities, Date date, boolean[] discards, Sort sort);

    @Query("{'doctorId': null, 'speciality': {$in: ?0}, 'updatedTime': {'$gt': ?1}, 'discarded': {$in: ?2}}")
    List<NotesCollection> findGlobalNotes(Collection<String> specialities, Date date, boolean[] discards, Sort sort);

    @Query("{'doctorId': ?0, 'locationId': ?1, 'hospitalId': ?2, 'updatedTime': {'$gt': ?3}, 'discarded': {$in: ?4}}")
    List<NotesCollection> findCustomNotes(ObjectId doctorId, ObjectId locationId, ObjectId hospitalId, Date date, boolean[] discards, Sort sort);

    @Query("{'doctorId': ?0, 'updatedTime': {'$gt': ?1}, 'discarded': {$in: ?2}}")
    List<NotesCollection> findCustomNotes(ObjectId doctorId, Date date, boolean[] discards, Sort sort);

    @Query("{'$or': [{'doctorId': ?0, 'speciality': {$in: ?1}, 'updatedTime': {'$gt': ?2}, 'discarded': {$in: ?3}},{'doctorId': null, 'speciality': {$in: ?1}, 'updatedTime': {'$gt': ?2},'discarded': {$in: ?3}}]}")
    List<NotesCollection> findCustomGlobalNotes(ObjectId doctorId, Collection<String> specialities, Date date, boolean[] discards, Sort sort);
	...
	@Query("{'id': {'$in' : ?0}}")
	List<NotesCollection> findAll(List<String> specialNotes);
}
