...
public interface ClinicalNotesRepository extends MongoRepository<ClinicalNotesCollection, ObjectId>, PagingAndSortingRepository<ClinicalNotesCollection, ObjectId> {

//    @Query(value = "{'doctorId' : ?0, 'hospitalId' : ?1, 'locationId' : ?2}", count = true)
//    Integer getClinicalNotesCount(ObjectId doctorId, ObjectId hospitalId, ObjectId locationId);
//
...
//
    ...
    @Query("{'doctorId' : ?0, 'patientId' : ?1, 'updatedTime' : {'$gt' : ?2}, 'discarded' : {$in: ?3}, 'inHistory' : {$in: ?4}}")
	List<ClinicalNotesCollection> getClinicalNotes(ObjectId doctorId, ObjectId patientId, Date date, boolean[] discards, boolean[] inHistorys, Sort sort);

	@Query("{'doctorId' : ?0, 'hospitalId' : ?1, 'locationId' : ?2, 'patientId' : ?3, 'updatedTime' : {'$gt' : ?4}, 'discarded' : {$in: ?5}, 'inHistory' : {$in: ?6}}")
	List<ClinicalNotesCollection> getClinicalNotes(ObjectId doctorId, ObjectId hospitalId, ObjectId locationId, ObjectId patientId, Date date, boolean[] discards, boolean[] inHistorys, Pageable pageable);

	@Query("{'doctorId' : ?0, 'hospitalId' : ?1, 'locationId' : ?2, 'patientId' : ?3, 'updatedTime' : {'$gt' : ?4}, 'discarded' : {$in: ?5}, 'inHistory' : {$in: ?6}}")
	List<ClinicalNotesCollection> getClinicalNotes(ObjectId doctorId, ObjectId hospitalId, ObjectId locationId, ObjectId patientId, Date date, boolean[] discards, boolean[] inHistorys, Sort sort);

	@Query("{'patientId' : ?0, 'updatedTime' : {'$gt' : ?1}, 'discarded' : {$in: ?2}, 'inHistory' : {$in: ?3}}")
	List<ClinicalNotesCollection> getClinicalNotes(ObjectId patientId, Date date, boolean[] discards,	boolean[] inHistorys, Pageable pageable);

	@Query("{'patientId' : ?0, 'updatedTime' : {'$gt' : ?1}, 'discarded' : {$in: ?2}, 'inHistory' : {$in: ?3}}")
	List<ClinicalNotesCollection> getClinicalNotes(ObjectId patientId, Date date, boolean[] discards,	boolean[] inHistorys, Sort sort);
	...
	Integer getClinicalNotesCount(ObjectId doctorId, ObjectId patientId, ObjectId hospitalId, ObjectId locationId, boolean discarded);
