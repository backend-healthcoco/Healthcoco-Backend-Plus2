		...
		try {
			...
			if (html) {
				...
				try {
					...
					String oopath = uri + "/" + BusinessException.class.getSimpleName() + "/"
							+ data.getErrorCode().name() + "/index.jsp.oo";
				...
				return Response.status(data.getErrorCode().getErrorCode()).type(MediaType.APPLICATION_JSON_TYPE)
						.entity(jsonStr).build();
			} else if (xml) {
				String xmlStr = getXml(data);
				return Response.status(data.getErrorCode().getErrorCode()).type(MediaType.TEXT_XML_TYPE).entity(xmlStr)
						.build();
			} else {
				StringWriter sw = new StringWriter();
				PrintWriter pw = new PrintWriter(sw);
				exception.printStackTrace(pw);
				return Response.status(data.getErrorCode().getErrorCode()).type(MediaType.TEXT_XML_TYPE)
