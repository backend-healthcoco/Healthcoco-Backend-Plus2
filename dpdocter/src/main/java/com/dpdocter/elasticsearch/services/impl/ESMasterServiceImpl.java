...
public class ESMasterServiceImpl implements ESMasterService {
	...
	@Override
	public Response<Reference> searchReference(String range, int page, int size, String doctorId, String locationId,
	...
	private Response<Reference> getGlobalReferences(int page, int size, String updatedTime, Boolean discarded,
			String searchTerm) {
		...
		try {
			SearchQuery searchQuery = DPDoctorUtils.createGlobalQuery(Resource.REFERENCE, page, size, updatedTime,
					discarded, "reference", searchTerm, null, null, null, "reference");
	...
	private Response<Reference> getCustomReferences(int page, int size, String doctorId, String locationId,
			String hospitalId, String updatedTime, Boolean discarded, String searchTerm) {
		Response<Reference> response = new Response<Reference>();
		List<ESReferenceDocument> referenceDocuments = null;
		try {
			SearchQuery searchQuery = DPDoctorUtils.createCustomQuery(page, size, doctorId, locationId, hospitalId,
					updatedTime, discarded, "reference", searchTerm, null, null, "reference");
			referenceDocuments = elasticsearchTemplate.queryForList(searchQuery, ESReferenceDocument.class);
			Integer count = (int) elasticsearchTemplate.count(
					new NativeSearchQueryBuilder().withQuery(searchQuery.getQuery()).build(),
					ESReferenceDocument.class);
	...
	private Response<Reference> getCustomGlobalReferences(int page, int size, String doctorId, String locationId,
			String hospitalId, String updatedTime, Boolean discarded, String searchTerm) {
		Response<Reference> response = new Response<Reference>();
		List<ESReferenceDocument> referenceDocuments = null;
		try {
			SearchQuery searchQuery = DPDoctorUtils.createCustomGlobalQuery(Resource.REFERENCE, page, size, doctorId,
					locationId, hospitalId, updatedTime, discarded, "reference", searchTerm, null, null, null,
					"reference");
			referenceDocuments = elasticsearchTemplate.queryForList(searchQuery, ESReferenceDocument.class);
			Integer count = (int) elasticsearchTemplate.count(
					new NativeSearchQueryBuilder().withQuery(searchQuery.getQuery()).build(),
	...
	@Override
	public List<DiseaseListResponse> searchDisease(String range, long page, int size, String doctorId,
	...
	private List<DiseaseListResponse> getCustomDiseases(long page, int size, String doctorId, String locationId,
			String hospitalId, String updatedTime, Boolean discarded, String searchTerm) {
		List<DiseaseListResponse> diseaseListResponses = null;
		...
		try {
			...
			if (DPDoctorUtils.anyStringEmpty(doctorId))
				...
				SearchQuery searchQuery = DPDoctorUtils.createCustomQuery(page, size, doctorId, locationId, hospitalId,
						updatedTime, discarded, "disease", searchTerm, null, null, "disease");
	...
	private List<DiseaseListResponse> getGlobalDiseases(long page, int size, String updatedTime, Boolean discarded,
			String searchTerm) {
		List<DiseaseListResponse> diseaseListResponses = null;
		List<ESDiseasesDocument> diseasesDocuments = null;
		try {
			...
			SearchQuery searchQuery = DPDoctorUtils.createGlobalQuery(Resource.DISEASE, page, size, updatedTime,
					discarded, "disease", searchTerm, null, null, null, "disease");
			diseasesDocuments = elasticsearchTemplate.queryForList(searchQuery, ESDiseasesDocument.class);
	...
	private List<DiseaseListResponse> getCustomGlobalDiseases(long page, int size, String doctorId, String locationId,
			String hospitalId, String updatedTime, Boolean discarded, String searchTerm) {
		List<DiseaseListResponse> diseaseListResponses = null;
		List<ESDiseasesDocument> diseasesDocuments = null;
		try {
			SearchQuery searchQuery = DPDoctorUtils.createCustomGlobalQuery(Resource.DISEASE, page, size, doctorId,
					locationId, hospitalId, updatedTime, discarded, "disease", searchTerm, null, null, null, "disease");
		...
		try {
			...
			if (size > 0)
				searchQuery = new NativeSearchQueryBuilder().withQuery(boolQueryBuilder)
						.withPageable(PageRequest.of((int) page, size, Direction.ASC, "profession")).build();
			else
				searchQuery = new NativeSearchQueryBuilder().withQuery(boolQueryBuilder)
	...
	public Boolean add() {
		...
		for (LandmarkLocalityCollection landmarkLocalityCollection : landmarkLocalityCollections) {
			...
			landmarkLocalityDocument.setGeoPoint(
					new GeoPoint(landmarkLocalityDocument.getLatitude(), landmarkLocalityDocument.getLongitude()));
	...
	@Override
	public List<ProfessionalMembership> searchProfessionalMembership(String searchTerm, String updatedTime, long page,
			int size) {
		...
		try {
			...
			if (size > 0)
				searchQuery = new NativeSearchQueryBuilder().withQuery(boolQueryBuilder)
						.withPageable(PageRequest.of((int) page, size, Direction.ASC, "membership")).build();
			else
				searchQuery = new NativeSearchQueryBuilder().withQuery(boolQueryBuilder)
	...
	@Override
	public List<EducationInstitute> searchEducationInstitute(String searchTerm, String updatedTime, long page,
			int size) {
		...
		try {
			...
			if (size > 0)
				searchQuery = new NativeSearchQueryBuilder().withQuery(boolQueryBuilder)
						.withPageable(PageRequest.of((int) page, size, Direction.ASC, "name")).build();
			else
				searchQuery = new NativeSearchQueryBuilder().withQuery(boolQueryBuilder)
	...
	@Override
	public List<EducationQualification> searchEducationQualification(String searchTerm, String updatedTime, long page,
			int size) {
	...
	public List<MedicalCouncil> searchMedicalCouncil(String searchTerm, String updatedTime, long page, int size) {
		...
		try {
			...
			if (size > 0)
				searchQuery = new NativeSearchQueryBuilder().withQuery(boolQueryBuilder)
						.withPageable(PageRequest.of((int) page, size, Direction.ASC, "medicalCouncil")).build();
			else
				searchQuery = new NativeSearchQueryBuilder().withQuery(boolQueryBuilder)
	...
	public List<Speciality> searchSpeciality(String searchTerm, String updatedTime, long page, int size) {
		...
		try {
			...
			if (size > 0)
				searchQuery = new NativeSearchQueryBuilder().withQuery(boolQueryBuilder)
						.withPageable(PageRequest.of((int) page, size, Direction.ASC, "superSpeciality")).build();
			else
				searchQuery = new NativeSearchQueryBuilder().withQuery(boolQueryBuilder)
						.withSort(SortBuilders.fieldSort("superSpeciality").order(SortOrder.ASC)).build();
	...
	@Override
	public void addEditSymptomDiseaseConditionDocument(
			ESSymptomDiseaseConditionDocument esSymptomDiseaseConditionDocument) {
		try {
			esSymptomDiseaseConditionRepository.save(esSymptomDiseaseConditionDocument);
			transactionalManagementService.addResource(new ObjectId(esSymptomDiseaseConditionDocument.getId()),
					Resource.SYMPTOM_DISEASE_CONDITION, true);
		} catch (Exception e) {
			e.printStackTrace();
