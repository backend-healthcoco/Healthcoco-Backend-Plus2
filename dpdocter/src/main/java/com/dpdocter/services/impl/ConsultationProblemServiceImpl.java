...
public class ConsultationProblemServiceImpl implements ConsultationProblemDetailsService {
	...
	public ConsultationProblemDetails addEditProblemDetails(ConsultationProblemDetailsRequest request) {
		...
		try {
			if (!DPDoctorUtils.anyStringEmpty(request.getId())) {
				consultationProblemDetailsCollection = consultationProblemDetailsRepository
			...
			throw new BusinessException(ServiceError.Unknown,
					"Error while add/edit consultation problem Details " + e.getMessage());
	...
	public ConsultationProblemDetails getProblemDetails(String problemDetailsId) {
		...
		try {
			...
			ConsultationProblemDetailsCollection problemDetailsCollection = consultationProblemDetailsRepository
					.findById(new ObjectId(problemDetailsId)).orElse(null);
			...
			e.printStackTrace();
			throw new BusinessException(ServiceError.Unknown,
					"Error while getting consultation problem details " + e.getMessage());
	...
	public Integer countConsultationProblemDetails(Boolean discarded, String searchTerm) {
		...
		try {
			...
			e.printStackTrace();
			throw new BusinessException(ServiceError.Unknown,
					"Error while counting Consultation Problem Details " + e.getMessage());
