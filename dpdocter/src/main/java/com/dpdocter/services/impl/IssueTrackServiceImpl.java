...
//		mailService.sendEmail(userCollection.getEmailAddress(), addIssueSubject, body, null);
		pushNotificationServices.notifyUser(userCollection.getId().toString(), request.getLocationId(), request.getHospitalId(), "Your issue "+issueTrackCollection.getIssueCode()+" has been recorded, we will keep you updated on the progress of the issue", null, null, null, null, null, null);
	    }
		...
		if (!IssueStatus.valueOf(status.toUpperCase()).equals(IssueStatus.REOPEN)) {
		    ...
		    if(userCollection != null){
		    	if(status.equalsIgnoreCase(IssueStatus.INPROGRESS.getStatus()))
			    	pushNotificationServices.notifyUser(userCollection.getId().toString(), null, null, "We have started working on the issue "+issueTrackCollection.getIssueCode()+", we will keep you upadated on the progress of the issue", null, null, null, null, null, null);
		    	else if(status.equalsIgnoreCase(IssueStatus.COMPLETED.getStatus()))
		    		pushNotificationServices.notifyUser(userCollection.getId().toString(), null, null, "Your issue "+issueTrackCollection.getIssueCode()+" has been resolved, please let us know if you are not satisfied; we will be happy to look at it again", null, null, null, null, null, null);
