...
//			jrDesignTextField.setUnderline(true);
		...
		band.setHeight(20);
		band.setPrintWhenExpression(new JRDesignExpression(
				"(!$F{personalHistoryAlcohol}.equals( null ) && ! $F{personalHistoryAlcohol} .isEmpty()) || "
				+ "(!$F{personalHistoryTobacco}.equals( null ) && ! $F{personalHistoryTobacco} .isEmpty())"
						+ "|| (!$F{personalHistorySmoking}.equals( null ) && ! $F{personalHistorySmoking} .isEmpty())"
						+ "|| (!$F{personalHistoryDiet}.equals( null ) && ! $F{personalHistoryDiet} .isEmpty())"
		...
		band.setHeight(20);
		band.setPrintWhenExpression(new JRDesignExpression(
				"(!$F{generalHistoryDrugs}.equals( null ) && ! $F{generalHistoryDrugs} .isEmpty())"
				+ "|| (!$F{generalHistoryMedicine}.equals( null ) && ! $F{generalHistoryMedicine} .isEmpty())"
						+ "|| (!$F{generalHistoryAllergies}.equals( null ) && ! $F{generalHistoryAllergies} .isEmpty())"
						+ "|| (!$F{generalHistorySurgical}.equals( null ) && ! $F{generalHistorySurgical} .isEmpty())"));
		jrDesignTextField = new JRDesignTextField();
		jrDesignTextField.setExpression(new JRDesignExpression("$P{GeneralHistoryTitle}"));
		jrDesignTextField.setX(1);
		jrDesignTextField.setY(0);
...
//		((JRDesignSection) jasperDesign.getDetailSection()).addBand(band);
	...
	private void createDischargeSummary(JasperDesign jasperDesign, Map<String, Object> parameters,
		...
		addItems(jasperDesign, columnWidth, "$P{WeightOnAdmission}", "$P{weightOnAdmission}", fieldWidth, false, 0, false);
		addItems(jasperDesign, columnWidth, "$P{WeightOnDischarge}", "$P{weightOnDischarge}", fieldWidth, false, 0, false);
	...
	private void createAdmitCard(JasperDesign jasperDesign, Map<String, Object> parameters, Integer contentFontSize,
		...
		if (show) {
			addItems(jasperDesign, columnWidth, "$P{NatureOfOperation}", "$P{natureOfOperation}", fieldWidth, false, 0, false);
		}
		...
		if (show) {
			addItems(jasperDesign, columnWidth, "$P{PersonalHistoryTitle}", "$P{personalHistory}", fieldWidth, false, 0, false);
		}
		...
		if (show) {
			addItems(jasperDesign, columnWidth, "$P{JointInvolvement}", "$P{jointInvolvement}", fieldWidth, false, 0, false);
		}
