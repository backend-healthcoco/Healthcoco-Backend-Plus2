...
public class BankDetailsServiceImpl implements BankDetailsService {
		...
		try {
			...
			if (!DPDoctorUtils.anyStringEmpty(request.getId())) {
				...
				UserCollection userCollection = userRepository.findById(new ObjectId(request.getDoctorId()))
						.orElse(null);
				mailService
						.sendEmail(mailRequest, " New Request for Online Consultation",
								"Doctor Name: " + userCollection.getFirstName() + " " + "EmailAddress: "
										+ userCollection.getEmailAddress() + " " + "DoctorId:" + userCollection.getId(),
								null);
			}
			bankDetailsCollection.setAccountholderName(
					AES.encrypt(bankDetailsCollection.getAccountholderName(), secretKeyAccountDetails));
			bankDetailsCollection
					.setAccountNumber(AES.encrypt(bankDetailsCollection.getAccountNumber(), secretKeyAccountDetails));
			bankDetailsCollection
					.setIfscNumber(AES.encrypt(bankDetailsCollection.getIfscNumber(), secretKeyAccountDetails));
			bankDetailsCollection
					.setPanCardNumber(AES.encrypt(bankDetailsCollection.getPanCardNumber(), secretKeyAccountDetails));
			bankDetailsCollection
					.setBankName(AES.encrypt(bankDetailsCollection.getBankName(), secretKeyAccountDetails));
			bankDetailsCollection
					.setBranchCity(AES.encrypt(bankDetailsCollection.getBranchCity(), secretKeyAccountDetails));
	...
	public BankDetails getBankDetailsByDoctorId(String doctorId) {
		...
		try {
			BankDetailsCollection bankDetailsCollection = bankDetailsRepository.findByDoctorId(new ObjectId(doctorId))
					.orElse(null);

			if (bankDetailsCollection != null) {

				bankDetailsCollection.setAccountholderName(
						AES.decrypt(bankDetailsCollection.getAccountholderName(), secretKeyAccountDetails));
				bankDetailsCollection.setAccountNumber(
						AES.decrypt(bankDetailsCollection.getAccountNumber(), secretKeyAccountDetails));
				bankDetailsCollection
						.setIfscNumber(AES.decrypt(bankDetailsCollection.getIfscNumber(), secretKeyAccountDetails));
				bankDetailsCollection.setPanCardNumber(
						AES.decrypt(bankDetailsCollection.getPanCardNumber(), secretKeyAccountDetails));
				bankDetailsCollection
						.setBankName(AES.decrypt(bankDetailsCollection.getBankName(), secretKeyAccountDetails));
				bankDetailsCollection
						.setBranchCity(AES.decrypt(bankDetailsCollection.getBranchCity(), secretKeyAccountDetails));
