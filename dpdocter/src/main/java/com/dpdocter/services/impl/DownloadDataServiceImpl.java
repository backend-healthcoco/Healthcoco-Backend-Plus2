...
//	@Scheduled(cron="00 00 3 * * *", zone="IST")
	...
	public void sendDataToDoctor() {
		...
		List<DownloadDataRequestCollection> downloadDataRequestCollections = mongoTemplate.aggregate(
				Aggregation.newAggregation(
						Aggregation.match(new Criteria("isMailSend").is(false).and("dataType").exists(true))),
		...
		if (downloadDataRequestCollections != null && !downloadDataRequestCollections.isEmpty())
			for (DownloadDataRequestCollection downloadDataRequestCollection : downloadDataRequestCollections) {
				...
				if (dataType.contains(ComponentType.PATIENT)) {
					mailAttachments.add(generatePatientData(downloadDataRequestCollection.getDoctorId(),
							downloadDataRequestCollection.getLocationId(),
							downloadDataRequestCollection.getHospitalId()));
				}
	...
	public MailAttachment generatePatientData(ObjectId doctorId, ObjectId locationId, ObjectId hospitalId) {
		...
		try {
			Criteria criteria = new Criteria("locationId").is(locationId).and("hospitalId").is(hospitalId)
			...
			Aggregation aggregation = Aggregation.newAggregation(Aggregation.match(criteria),
					...
					new CustomAggregationOperation(new Document("$unwind",
							new BasicDBObject("path", "$patientGroupCollections").append("preserveNullAndEmptyArrays",
									true))),

					Aggregation.lookup("group_cl", "patientGroupCollections.groupId", "_id", "groups"),
					new CustomAggregationOperation(new Document("$unwind",
							new BasicDBObject("path", "$groups").append("preserveNullAndEmptyArrays", true))),
					...
					new CustomAggregationOperation(new Document("$project",
							new BasicDBObject("_id", "$_id").append("registrationDate", "$registrationDate")
									.append("patientId", "$PID").append("localPatientName", "$localPatientName")
									.append("mobileNumber", "$user.mobileNumber")
									.append("emailAddress", "$emailAddress").append("secMobile", "$secMobile")
									.append("gender", "$gender").append("country", "$address.country")
									.append("city", "$address.city").append("state", "$address.state")
									.append("postalCode", "$address.postalCode").append("locality", "$address.locality")
									.append("landmarkDetails", "$address.landmarkDetails")
									.append("streetAddress", "$address.streetAddress").append("adhaarId", "$adhaarId")
									.append("panCardNumber", "$panCardNumber")
									.append("drivingLicenseId", "$drivingLicenseId")
									.append("insuranceId", "$insuranceId").append("dob", "$dob")
									.append("bloodGroup", "$bloodGroup").append("referredBy", "$reference.reference")
			...
			List<PatientDownloadData> patientDownloadDatas = mongoTemplate
					.aggregate(aggregation.withOptions(Aggregation.newAggregationOptions().allowDiskUse(true).build()),
							PatientCollection.class, PatientDownloadData.class)
					.getMappedResults();
			...
			for (PatientDownloadData patientDownloadData : patientDownloadDatas) {
				if (patientDownloadData.getDob() != null) {
					patientDownloadData.setDateOfBirth(
							patientDownloadData.getDob().getDays() + "/" + patientDownloadData.getDob().getMonths()
									+ "/" + patientDownloadData.getDob().getYears() + "/");
					if (patientDownloadData.getDob().getAge() != null)
...
//		    cell.setCellValue(field.get(obj)+"");
	...
	public Boolean downloadClinicalItems(String doctorId, String locationId, String hospitalId) {
		...
		try {
			...
			List<ComplaintCollection> complaintCollections = mongoTemplate
					.aggregate(Aggregation.newAggregation(Aggregation.match(criteria)), ComplaintCollection.class,
			...
			List<ObservationCollection> observationCollections = mongoTemplate.aggregate(
					...
					ObservationCollection.class, ObservationCollection.class).getMappedResults();
			...
			List<DiagnosisCollection> diagnosisCollections = mongoTemplate.aggregate(
					Aggregation.newAggregation(Aggregation.match(new Criteria("locationId").is(locationObjectId)
							.and("hospitalId").is(hospitalObjectId).and("doctorId").is(doctorObjectId))),
					DiagnosisCollection.class, DiagnosisCollection.class).getMappedResults();
			...
			List<NotesCollection> notesCollections = mongoTemplate.aggregate(
					Aggregation.newAggregation(Aggregation.match(new Criteria("locationId").is(locationObjectId)
							.and("hospitalId").is(hospitalObjectId).and("doctorId").is(doctorObjectId))),
					NotesCollection.class, NotesCollection.class).getMappedResults();
			...
			response = mailService.sendEmailMultiAttach("neha.pateliya@healthcoco.com",
					"Dr. Ajit Mahant's clinical notes items data", "Please find attachments", mailAttachments);
	...
	public MailAttachment downloadPrescriptionData(ObjectId doctorId, ObjectId locationId, ObjectId hospitalId) {
		...
		try {
			...
			while (isMorePresent) {
				...
				Aggregation aggregation = Aggregation.newAggregation(Aggregation.match(criteria),
						...
						new CustomAggregationOperation(new Document("$project",
								new BasicDBObject("_id", "$_id").append("doctorName", "$user.firstName")
										...
										.append("patientId", "$patient.PID").append("drugName", "$drug.drugName")
										.append("drugType", "$drug.drugType.type")
										.append("duration",
												new BasicDBObject("$concat",
														Arrays.asList("$items.duration.value", " ",
																"$items.duration.durationUnit.unit")))
										.append("dosage", "$items.dosage").append("explanation", "$explanation")
										.append("direction", "$items.direction.direction")
										.append("instructions", "$instructions")
										.append("diagnosticTests", "$diagnosticTests").append("advice", "$advice")
										.append("date",
				...
				List<PrescriptionDownloadData> prescriptionDownloadDatas = mongoTemplate.aggregate(
						aggregation.withOptions(Aggregation.newAggregationOptions().allowDiskUse(true).build()),
						PrescriptionCollection.class, PrescriptionDownloadData.class).getMappedResults();
				if (prescriptionDownloadDatas != null && !prescriptionDownloadDatas.isEmpty()) {
					...
					for (PrescriptionDownloadData prescriptionDownloadData : prescriptionDownloadDatas) {
						if (prescriptionDownloadData.getDiagnosticTests() != null
								&& !prescriptionDownloadData.getDiagnosticTests().isEmpty()) {
							String test = "";
							for (TestAndRecordData tests : prescriptionDownloadData.getDiagnosticTests()) {
								...
								if (diagnosticTestCollection != null) {
									if (DPDoctorUtils.anyStringEmpty(test))
										test = "'" + diagnosticTestCollection.getTestName();
									else
										test = test + "," + diagnosticTestCollection.getTestName();
								}
							}
							test = test + "'";
	...
	public MailAttachment downloadAppointmentData(ObjectId doctorId, ObjectId locationId, ObjectId hospitalId) {
		...
		try {
			Criteria criteria = new Criteria("locationId").is(locationId).and("hospitalId").is(hospitalId)
					.and("doctorId").is(doctorId).and("type").is(AppointmentType.APPOINTMENT.getType());
			...
			Aggregation aggregation = Aggregation.newAggregation(Aggregation.match(criteria),
					...
					new CustomAggregationOperation(new Document("$unwind",
							new Document("path", "$patient").append("preserveNullAndEmptyArrays", true))),
					new CustomAggregationOperation(new Document("$redact", new BasicDBObject("$cond",
							new BasicDBObject("if",
									new BasicDBObject("$eq", Arrays.asList("$patient.locationId", "$locationId")))
											.append("then", "$$KEEP").append("else", "$$PRUNE")))),
...
//					new CustomAggregationOperation(new Document("$sort", new BasicDBObject("date", 1)))
			...
			List<AppointmentDownloadData> appointmentDownloadDatas = mongoTemplate
					.aggregate(aggregation.withOptions(Aggregation.newAggregationOptions().allowDiskUse(true).build()),
							AppointmentCollection.class, AppointmentDownloadData.class)
					.getMappedResults();
		...
		try {
			...
			Aggregation aggregation = Aggregation.newAggregation(Aggregation.match(criteria),
					...
					Aggregation.lookup("treatment_services_cl", "treatments.treatmentServiceId", "_id",
							"treatmentService"),
					new CustomAggregationOperation(new Document("$unwind",
							new BasicDBObject("path", "$treatmentService").append("preserveNullAndEmptyArrays", true))),
					...
					new CustomAggregationOperation(new Document("$project", new BasicDBObject("_id", "$_id")
							...
							.append("date",
									new BasicDBObject("$dateToString",
											new BasicDBObject("format", "%Y-%m-%d").append("date", "$createdTime")))
							.append("treatmentName", "$treatmentService.name").append("status", "$treatments.status")
							.append("cost", "$treatments.cost").append("quantity", "$treatments.quantity.value")
							...
							.append("finalCost", "$treatments.finalCost").append("note", "$treatments.note"))),

					new CustomAggregationOperation(new Document("$sort", new BasicDBObject("date", 1))));

			List<TreatmentDownloadData> treatmentDownloadDatas = mongoTemplate
					.aggregate(aggregation.withOptions(Aggregation.newAggregationOptions().allowDiskUse(true).build()),
							PatientTreatmentCollection.class, TreatmentDownloadData.class)
					.getMappedResults();
	...
	public MailAttachment downloadClinicalNotesData(ObjectId doctorId, ObjectId locationId, ObjectId hospitalId) {
		...
		try {
			...
			DynamicUICollection dynamicUICollection = mongoTemplate
					.aggregate(Aggregation.newAggregation(Aggregation.match(criteria)), DynamicUICollection.class,
							DynamicUICollection.class)
					.getUniqueMappedResult();

			List<String> clinicalNotesPermission = null;
			if (dynamicUICollection != null && dynamicUICollection.getUiPermissions() != null
					&& dynamicUICollection.getUiPermissions().getClinicalNotesPermissions() != null
					&& !dynamicUICollection.getUiPermissions().getClinicalNotesPermissions().isEmpty()) {
				clinicalNotesPermission = dynamicUICollection.getUiPermissions().getClinicalNotesPermissions();
			}

			Aggregation aggregation = Aggregation.newAggregation(Aggregation.match(criteria), Aggregation.lookup(
					"user_cl", "doctorId", "_id", "user"), Aggregation.unwind("user"),
					new CustomAggregationOperation(new Document("$unwind", new BasicDBObject("path", "$user").append(
							"preserveNullAndEmptyArrays", true))),
					Aggregation.lookup("patient_cl", "patientId", "userId", "patient"), new CustomAggregationOperation(
							new Document("$unwind", new BasicDBObject("path", "$patient").append(
									"preserveNullAndEmptyArrays", true))),
					new CustomAggregationOperation(new Document("$redact", new BasicDBObject("$cond",
							new BasicDBObject("if",
									new BasicDBObject("$eq", Arrays.asList("$patient.locationId", "$locationId")))
					...
					new CustomAggregationOperation(new Document("$project", new BasicDBObject("_id", "$_id")
							...
							.append("date",
									new BasicDBObject("$dateToString",
											new BasicDBObject("format", "%Y-%m-%d").append("date", "$createdTime")))
							.append("comments", "$comments").append("pulse", "$vitalSigns.pulse")
							.append("temperature", "$vitalSigns.temperature")
							.append("breathing", "$vitalSigns.breathing")
							.append("bloodPressure",
									new BasicDBObject("$concat",
											Arrays.asList("$vitalSigns.bloodPressure.systolic", "/",
													"$vitalSigns.bloodPressure.diastolic")))
							.append("height", "$vitalSigns.height").append("weight", "$vitalSigns.weight")
							.append("spo2", "$vitalSigns.spo2").append("bmi", "$vitalSigns.bmi")
							.append("bsa", "$vitalSigns.bsa").append("complaint", "$complaint")
							.append("observation", "$observation").append("investigation", "$investigation")
							.append("diagnosis", "$diagnosis").append("note", "$note")
							.append("provisionalDiagnosis", "$provisionalDiagnosis")
							.append("generalExam", "$generalExam").append("systemExam", "$systemExam")
							...
							.append("ps", "$ps").append("ecgDetails", "$ecgDetails")
							.append("xRayDetails", "$xRayDetails").append("echo", "$echo").append("holter", "$holter")
							.append("pcNose", "$pcNose").append("pcOralCavity", "$pcOralCavity")
							.append("pcThroat", "$pcThroat").append("pcEars", "$pcEars").append("noseExam", "$noseExam")
							.append("oralCavityThroatExam", "$oralCavityThroatExam")
							.append("indirectLarygoscopyExam", "$indirectLarygoscopyExam")
							.append("neckExam", "$neckExam").append("earsExam", "$earsExam"))),
					new CustomAggregationOperation(new Document("$sort", new BasicDBObject("date", 1))));

			List<ClinicalNotesDownloadData> clinicalNotesDownloadDatas = mongoTemplate
					.aggregate(aggregation.withOptions(Aggregation.newAggregationOptions().allowDiskUse(true).build()),
							ClinicalNotesCollection.class, ClinicalNotesDownloadData.class)
					.getMappedResults();
			...
			if (clinicalNotesPermission != null && !clinicalNotesPermission.isEmpty()) {
				if (clinicalNotesPermission.contains(ClinicalNotesPermissionEnum.VITAL_SIGNS.getPermissions())) {
					clinicalNotesFields.add("pulse");
					clinicalNotesFields.add("temperature");
					clinicalNotesFields.add("breathing");
					clinicalNotesFields.add("bloodPressure");
					clinicalNotesFields.add("height");
					clinicalNotesFields.add("weight");
					clinicalNotesFields.add("spo2");
					clinicalNotesFields.add("bmi");
					clinicalNotesFields.add("bsa");
				}
				if (clinicalNotesPermission.contains(ClinicalNotesPermissionEnum.COMPLAINT.getPermissions()))
					clinicalNotesFields.add("complaint");
				if (clinicalNotesPermission.contains(ClinicalNotesPermissionEnum.OBSERVATION.getPermissions()))
					clinicalNotesFields.add("observation");
				if (clinicalNotesPermission.contains(ClinicalNotesPermissionEnum.INVESTIGATIONS.getPermissions()))
					clinicalNotesFields.add("investigation");
				if (clinicalNotesPermission.contains(ClinicalNotesPermissionEnum.DIAGNOSIS.getPermissions()))
					clinicalNotesFields.add("diagnosis");
				if (clinicalNotesPermission.contains(ClinicalNotesPermissionEnum.NOTES.getPermissions()))
					clinicalNotesFields.add("note");
				if (clinicalNotesPermission
						.contains(ClinicalNotesPermissionEnum.PROVISIONAL_DIAGNOSIS.getPermissions()))
					clinicalNotesFields.add("provisionalDiagnosis");
				if (clinicalNotesPermission.contains(ClinicalNotesPermissionEnum.GENERAL_EXAMINATION.getPermissions()))
					clinicalNotesFields.add("generalExam");
				if (clinicalNotesPermission.contains(ClinicalNotesPermissionEnum.SYSTEMIC_EXAMINATION.getPermissions()))
					clinicalNotesFields.add("systemExam");
				if (clinicalNotesPermission.contains(ClinicalNotesPermissionEnum.PRESENT_COMPLAINT.getPermissions()))
					clinicalNotesFields.add("presentComplaint");
				if (clinicalNotesPermission
						.contains(ClinicalNotesPermissionEnum.HISTORY_OF_PRESENT_COMPLAINT.getPermissions()))
					clinicalNotesFields.add("presentComplaintHistory");
				if (clinicalNotesPermission.contains(ClinicalNotesPermissionEnum.MENSTRUAL_HISTORY.getPermissions()))
					clinicalNotesFields.add("menstrualHistory");
				if (clinicalNotesPermission.contains(ClinicalNotesPermissionEnum.OBSTETRIC_HISTORY.getPermissions()))
					clinicalNotesFields.add("obstetricHistory");
			} else {
				clinicalNotesFields.add("pulse");
				clinicalNotesFields.add("temperature");
				clinicalNotesFields.add("breathing");
	...
	public MailAttachment downloadInvoicesData(ObjectId doctorId, ObjectId locationId, ObjectId hospitalId) {
		...
		try {
			...
			List<InvoiceDownloadData> treatmentDownloadDatas = mongoTemplate
					.aggregate(aggregation.withOptions(Aggregation.newAggregationOptions().allowDiskUse(true).build()),
							DoctorPatientInvoiceCollection.class, InvoiceDownloadData.class)
					.getMappedResults();
			...
			Aggregation aggregation = Aggregation.newAggregation(Aggregation.match(criteria),
					...
					new CustomAggregationOperation(new Document("$project", new BasicDBObject("_id", "$_id")
							...
							.append("date",
									new BasicDBObject("$dateToString",
											new BasicDBObject("format", "%Y-%m-%d").append("date", "$createdTime")))
							.append("receiptId", "$uniqueReceiptId").append("invoiceId", "$uniqueInvoiceId")
							.append("modeOfPayment", "$modeOfPayment").append("amountPaid", "$amountPaid"))),

					new CustomAggregationOperation(new Document("$sort", new BasicDBObject("date", 1))));

			List<ReceiptDownloadData> receiptDownloadDatas = mongoTemplate
					.aggregate(aggregation.withOptions(Aggregation.newAggregationOptions().allowDiskUse(true).build()),
							DoctorPatientReceiptCollection.class, ReceiptDownloadData.class)
					.getMappedResults();
	...
	public Boolean update(String doctorId, String locationId, String hospitalId) {
		Boolean response = false;
		try {
			List<DoctorPatientReceiptCollection> doctorPatientReceiptCollections = mongoTemplate
					.aggregate(
							Aggregation.newAggregation(
									Aggregation.match(new Criteria("doctorId").is(new ObjectId(doctorId))
											.and("locationId").is(new ObjectId(locationId)).and("hospitalId")
											.is(new ObjectId(hospitalId)).and("uniqueReceiptId").is(null)),
									Aggregation.sort(new Sort(Direction.ASC, "createdTime"))),
							DoctorPatientReceiptCollection.class, DoctorPatientReceiptCollection.class)
					.getMappedResults();
...
//			throw new BusinessException(ServiceError.Unknown, "Error downloading receipt data");
	...
	@Override
	public Boolean downloadfiles(String doctorId, String locationId, String hospitalId, int page, int size) {
		Boolean response = false;
		CSVWriter csvWriter = null;
...
//		String dst = "drname/patientId/filename.jpg";
		...
		try {
			...
			List<RecordsLookupResponse> downloadDatas = null;
			if (size != 0) {
				downloadDatas = mongoTemplate
						.aggregate(aggregation, RecordsCollection.class, RecordsLookupResponse.class)
						.getMappedResults();
			}
			if (downloadDatas.size() != 0)
				for (RecordsLookupResponse patientDownloadData : downloadDatas) {
					if (patientDownloadData.getRecordsUrl() != null) {
						System.out.println(patientDownloadData.getRecordsUrl());
						System.out.println("step0");
						CopyObjectRequest req = new CopyObjectRequest(bkt, patientDownloadData.getRecordsUrl(), bkt,
								path + patientDownloadData.getRecordsUrl());
