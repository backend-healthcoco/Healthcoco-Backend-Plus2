...
public class NutritionEngineServiceImpl implements NutritionEngineService {
	...
	public boolean compareNutrients(Map<String, String> nutrient1, Map<String, Double> map, int perOfValue){
		...
		if(nutrient1 == null && map == null) return true;
		if(nutrient1 == null || map == null) return false;
		
		map.forEach((key, value) -> System.out.println(key + ":" + value));

		//if(nutrient1.size() != map.size())return isMatched;
	...
	private boolean compareNutrientValues(String value1, Double val2, int perOfValue) {
		...
		if(DPDoctorUtils.anyStringEmpty(value1))return true;
		...
		if(isInRange((perOfValue * val1)/100, val2)) return true;
	...
	private boolean isInRange(Double rangeValue, Double valueToBeCheck) {
		...
		System.out.println((rangeValue-fivePerValue)+ " <= "+valueToBeCheck +" <= "+(rangeValue+fivePerValue));
	 ...
	 */
	//@Override
	public boolean bodyParametersNutrientsMatchesRecipeNutrients(NutritionRDA nutritionRDA, CombineRecipeNutrients recipeCollection, int perOfValue) {
		// TODO Auto-generated method stub
		
//		if(!compareNutrients(nutritionRDA.getGeneralNutrients(), recipeCollection.getGeneralNutrients(), perOfValue)) return false;		
//		if(!compareNutrients(nutritionRDA.getCarbNutrients(), recipeCollection.getCarbNutrients(), perOfValue)) return false;
//		if(!compareNutrients(nutritionRDA.getLipidNutrients(), recipeCollection.getLipidNutrients(), perOfValue)) return false;
//		if(!compareNutrients(nutritionRDA.getProteinAminoAcidNutrients(), recipeCollection.getProteinAminoAcidNutrients(), perOfValue)) return false;
//		if(!compareNutrients(nutritionRDA.getVitaminNutrients(), recipeCollection.getVitaminNutrients(), perOfValue)) return false;
//		if(!compareNutrients(nutritionRDA.getMineralNutrients(), recipeCollection.getMineralNutrients(), perOfValue)) return false;
//		if(!compareNutrients(nutritionRDA.getOtherNutrients(), recipeCollection.getOtherNutrients(), perOfValue)) return false;
		...
		Double proteins = Double.parseDouble(nutritionRDA.getProteinAminoAcidNutrients().getOrDefault("Protein", "0.0"));
...
//		System.out.println("RDA carbs: " + carbs +" fats: "+ fats+" proteins: "+proteins);
		System.out.println("recipeCarbs: " + recipeCarbs +" recipeFats: "+ recipeFats+" recipeProteins: "+recipeProteins);
		...
		if(isInRange(rangeValue, recipeCarbs)) {
			System.out.println("carbs in range");
		...
		if(isInRange(rangeValue, recipeFats)) {
			System.out.println("fats in range");
		}else {
			isMatch = false;
		}
		...
		if(isInRange(rangeValue, recipeProteins)) {
			System.out.println("proteins in range");
	 ...
	 */
//	@Override
	public DietPlan recipeSelection(String userId, List<MealTimeEnum> mealTimes, String doctorId, String locationId, String hospitalId) {
		// TODO Auto-generated method stub
...
//		nutritionRDA.getOtherNutrients().forEach((key, value) -> System.out.println(key + ":" + value));
	...
	private List<DietPlanRecipeAddItem> getRecipes(MealTimeEnum mealTimeEnum, NutritionRDA nutritionRDA) {
		...
		try {
			...
			case EARLY_MORNING:{
				...
				if(recipeCollections != null && recipeCollections.isEmpty())Collections.shuffle(recipeCollections);
				perOfValue = 10;
				System.out.println("EARLY_MORNING : %of RDA = "+perOfValue+" Total Recipes= " + (recipeCollections!= null ? recipeCollections.size():0));
				...
				for(RecipeCollection recipeCollection: recipeCollections) {
					...
					forCheck.setProteinAminoAcidNutrients(addNutrientsValue(recipeCollection.getProteinAminoAcidNutrients()));
			...
			case MID_MORNING:{
				...
				List<RecipeCollection> recipeCollections = recipeRepository.findByMealTiming(mealTimeEnum.getTime());
				if(recipeCollections != null && recipeCollections.isEmpty())Collections.shuffle(recipeCollections);
				System.out.println("MID_MORNING : %of RDA = "+perOfValue+" Total Recipes= " + (recipeCollections!= null ? recipeCollections.size():0));
			...
			case POST_LUNCH:{
				...
				List<RecipeCollection> recipeCollections = recipeRepository.findByMealTiming(mealTimeEnum.getTime());
				if(recipeCollections != null && recipeCollections.isEmpty())Collections.shuffle(recipeCollections);
				System.out.println("POST_LUNCH : %of RDA = "+perOfValue+" Total Recipes= " + (recipeCollections!= null ? recipeCollections.size():0));
			...
			case EVENING_SNACK:{
				perOfValue = 10;
				List<RecipeCollection> recipeCollections = recipeRepository.findByMealTimingAndFoodGroupsValueIn(mealTimeEnum.getTime(), 
						Arrays.asList("Milk And Milk Products", "Other Vegetables", "Nut", "Leafy Vegetables", "Fruits"));
				if(recipeCollections != null && recipeCollections.isEmpty())Collections.shuffle(recipeCollections);
				System.out.println("EVENING_SNACK : %of RDA = "+perOfValue+" Total Recipes= "  + (recipeCollections!= null ? recipeCollections.size():0));
			...
			case POST_DINNER:{
				...
				List<RecipeCollection> recipeCollections = recipeRepository.findByMealTiming(mealTimeEnum.getTime());
				if(recipeCollections != null && recipeCollections.isEmpty())Collections.shuffle(recipeCollections);
				System.out.println("POST_DINNER : %of RDA = "+perOfValue+" Total Recipes= "  + (recipeCollections!= null ? recipeCollections.size():0));
				for(RecipeCollection recipeCollection: recipeCollections) {
					System.out.println("Recipe Name : "  + recipeCollection.getName());
					CombineRecipeNutrients forCheck = new CombineRecipeNutrients();
					forCheck.setGeneralNutrients(addNutrientsValue(recipeCollection.getGeneralNutrients()));
	...
	private void getLunchSelectedRecipes(MealTimeEnum mealTimeEnum, List<DietPlanRecipeAddItem> recipeAddItems,
		...
		try {
			List<RecipeCollection> mainCourseRecipes = recipeRepository.findByMealTimingAndFoodGroupsValueRegex(mealTimeEnum.getTime(), "Cereal");
			if(mainCourseRecipes != null && !mainCourseRecipes.isEmpty())
				Collections.shuffle(mainCourseRecipes);
			System.out.println(mealTimeEnum +  ": %of RDA = "+perOfValue+" Total mainCourseRecipes= " + (mainCourseRecipes!= null ? mainCourseRecipes.size():0));
					
			List<RecipeCollection> sideCourseRecipes = recipeRepository.findByFoodGroupsValueIn(Arrays.asList("Pulses"));
			
			if(sideCourseRecipes != null && !sideCourseRecipes.isEmpty())
				Collections.shuffle(sideCourseRecipes);
			
			List<RecipeCollection> vegetablesRecipes = recipeRepository.findByMealTimingAndFoodGroupsValueIn(mealTimeEnum.getTime(), Arrays.asList("Other Vegetables", "Leafy Vegetables"));
			...
			if(vegetablesRecipes != null && !vegetablesRecipes.isEmpty()) {
				...
				if(vegetablesRecipes.size()>1)vegetablesRecipe1 = vegetablesRecipes.get(1);
			}
			
			RecipeCollection curdRecipe = recipeRepository.findByNameRegex("Curd Lassi");
			System.out.println(mealTimeEnum +  ": Total sideCourseRecipes" + (sideCourseRecipes!= null ? sideCourseRecipes.size():0));
			...
			for(RecipeCollection recipeCollection: mainCourseRecipes) {
				...
				if(sideCourseRecipes != null && !sideCourseRecipes.isEmpty()) {
					for(RecipeCollection sideDish : sideCourseRecipes) {
						...
						System.out.print(", vegetablesRecipe1 : " + (vegetablesRecipe1 !=null ? vegetablesRecipe1.getName() :""));
						System.out.print(", vegetablesRecipe2 : " + (vegetablesRecipe2 !=null ? vegetablesRecipe2.getName() :""));
						System.out.print(", curdRecipe : " + (curdRecipe !=null ? curdRecipe.getName() :""));
						
						System.out.println(" and " + sideDish.getName());
						CombineRecipeNutrients forCheck = addNutrientsValue(recipeCollection, sideDish, vegetablesRecipe1, vegetablesRecipe2, curdRecipe);
						if(bodyParametersNutrientsMatchesRecipeNutrients(nutritionRDA, forCheck, perOfValue)) {
							isMatched = true;
							DietPlanRecipeAddItem recipe = new DietPlanRecipeAddItem();
					...
					CombineRecipeNutrients forCheck = addNutrientsValue(recipeCollection, vegetablesRecipe1, vegetablesRecipe2, curdRecipe);
	...
	private CombineRecipeNutrients addNutrientsValue(RecipeCollection... recipes) {
		...
		for(RecipeCollection recipe : recipes) {
			if(recipe != null) {
				recipe.getGeneralNutrients().forEach((key, value) -> generalNutrients.put(key, generalNutrients.getOrDefault(key, 0.0) + Double.parseDouble(value)));
				recipe.getCarbNutrients().forEach((key, value) -> carbNutrients.put(key, carbNutrients.getOrDefault(key, 0.0) + Double.parseDouble(value)));
				recipe.getLipidNutrients().forEach((key, value) -> lipidNutrients.put(key, lipidNutrients.getOrDefault(key, 0.0) + Double.parseDouble(value)));
				recipe.getProteinAminoAcidNutrients().forEach((key, value) -> proteinAminoAcidNutrients.put(key, proteinAminoAcidNutrients.getOrDefault(key, 0.0) + Double.parseDouble(value)));
				recipe.getVitaminNutrients().forEach((key, value) -> vitaminNutrients.put(key, vitaminNutrients.getOrDefault(key, 0.0) + Double.parseDouble(value)));
				recipe.getMineralNutrients().forEach((key, value) -> mineralNutrients.put(key, mineralNutrients.getOrDefault(key, 0.0) + Double.parseDouble(value)));
				recipe.getOtherNutrients().forEach((key, value) -> otherNutrients.put(key, otherNutrients.getOrDefault(key, 0.0) + Double.parseDouble(value)));
			}	
		}
	...
	private void getBreakFastSelectedRecipes(MealTimeEnum mealTimeEnum, List<DietPlanRecipeAddItem> recipeAddItems,
		...
		List<RecipeCollection> recipesSide2 = recipeRepository.findByFoodGroupsValueIn(Arrays.asList("Milk And Milk Products", "Sugar"));
		...
		for(RecipeCollection recipeCollection: mainCourseRecipes) {
			...
			if(sideCourseRecipes != null && !sideCourseRecipes.isEmpty()) {
				for(RecipeCollection sideDish : sideCourseRecipes) {
					System.out.println("Recipe Name : " + recipeCollection.getName() + " and " + sideDish.getName());
					CombineRecipeNutrients forCheck = new CombineRecipeNutrients();
					forCheck.setGeneralNutrients(addNutrientsValue(recipeCollection.getGeneralNutrients(), sideDish.getGeneralNutrients(), (recipeSide2!=null?recipeSide2.getGeneralNutrients() : new HashMap<String, String>())));
					forCheck.setCarbNutrients(addNutrientsValue(recipeCollection.getCarbNutrients(), sideDish.getCarbNutrients(), (recipeSide2!=null?recipeSide2.getCarbNutrients() : new HashMap<String, String>())));
					forCheck.setLipidNutrients(addNutrientsValue(recipeCollection.getLipidNutrients(), sideDish.getLipidNutrients(), (recipeSide2!=null?recipeSide2.getLipidNutrients() : new HashMap<String, String>())));
					forCheck.setProteinAminoAcidNutrients(addNutrientsValue(recipeCollection.getProteinAminoAcidNutrients(), sideDish.getProteinAminoAcidNutrients(), (recipeSide2!=null?recipeSide2.getProteinAminoAcidNutrients() : new HashMap<String, String>())));
					forCheck.setVitaminNutrients(addNutrientsValue(recipeCollection.getVitaminNutrients(), sideDish.getVitaminNutrients(), (recipeSide2!=null?recipeSide2.getVitaminNutrients() : new HashMap<String, String>())));
					forCheck.setMineralNutrients(addNutrientsValue(recipeCollection.getMineralNutrients(), sideDish.getMineralNutrients(), (recipeSide2!=null?recipeSide2.getMineralNutrients() : new HashMap<String, String>())));
					forCheck.setOtherNutrients(addNutrientsValue(recipeCollection.getOtherNutrients(), sideDish.getOtherNutrients(), (recipeSide2!=null?recipeSide2.getOtherNutrients() : new HashMap<String, String>())));
	...
	private void getSelectedRecipes(MealTimeEnum mealTimeEnum, List<DietPlanRecipeAddItem> recipeAddItems, NutritionRDA nutritionRDA, int perOfValue) {
		try {
			List<RecipeCollection> mainCourseRecipes = recipeRepository.findByMealTimingAndFoodGroupsValueRegex(mealTimeEnum.getTime(), "Cereal");
			if(mainCourseRecipes != null && mainCourseRecipes.isEmpty())
				Collections.shuffle(mainCourseRecipes);
...
//			List<RecipeCollection> sideCourseRecipes = recipeRepository.findByMealTimingAndFoodGroupsValueIn(mealTimeEnum.getTime(), 
//					Arrays.asList("Milk And Milk Products", "Other Vegetables", "Pulses", "Leafy Vegetables"));
			
			List<RecipeCollection> sideCourseRecipes = recipeRepository.findByFoodGroupsValueIn(Arrays.asList("Pulses"));
//			List<RecipeCollection> sideCourseRecipes = recipeRepository.findByFoodGroupsValueIn(Arrays.asList("Milk And Milk Products", "Other Vegetables", "Pulses", "Leafy Vegetables"));
			...
			for(RecipeCollection recipeCollection: mainCourseRecipes) {
				...
				if(sideCourseRecipes != null && !sideCourseRecipes.isEmpty()) {
					for(RecipeCollection sideDish : sideCourseRecipes) {
						System.out.println(" and " + sideDish.getName());
						CombineRecipeNutrients forCheck = new CombineRecipeNutrients();
						forCheck.setGeneralNutrients(addNutrientsValue(recipeCollection.getGeneralNutrients(), sideDish.getGeneralNutrients()));
						forCheck.setCarbNutrients(addNutrientsValue(recipeCollection.getCarbNutrients(), sideDish.getCarbNutrients()));
						forCheck.setLipidNutrients(addNutrientsValue(recipeCollection.getLipidNutrients(), sideDish.getLipidNutrients()));
						forCheck.setProteinAminoAcidNutrients(addNutrientsValue(recipeCollection.getProteinAminoAcidNutrients(), sideDish.getProteinAminoAcidNutrients()));
						forCheck.setVitaminNutrients(addNutrientsValue(recipeCollection.getVitaminNutrients(), sideDish.getVitaminNutrients()));
						forCheck.setMineralNutrients(addNutrientsValue(recipeCollection.getMineralNutrients(), sideDish.getMineralNutrients()));
						forCheck.setOtherNutrients(addNutrientsValue(recipeCollection.getOtherNutrients(), sideDish.getOtherNutrients()));
	...
	private Map<String, Double> addNutrientsValue(Map<String, String>... nutrients) {
		...
		for(Map<String, String> nutrient : nutrients) {
			nutrient.forEach((key, value) -> response.put(key, response.getOrDefault(key, 0.0) + Double.parseDouble(value)));
		}
		return response;
	}

	@Override
	public DietPlan getRecipes(String userId, List<MealTimeEnum> mealTime, 
