...
public class DiagnosticTestOrderServicesimpl implements DiagnosticTestOrderService {
	...
	@Override
	public List<LabSearchResponse> searchLabs(String city, String location, String latitude, String longitude, String searchTerm, List<String> testNames,
			int page, int size, Boolean havePackage) {
		...
		try {
			...
			if(isLab || isUser) {
				projectList = new CustomAggregationOperation(new Document("$project", new BasicDBObject("_id", "$_id")
						...
						.append("pickUpTime","$pickUpTime")
						.append("pickUpDate","$pickUpDate")
						.append("testsPackageIds","$testsPackageIds")
						.append("diagnosticTests","$diagnosticTests")
						.append("pickUpAddress","$pickUpAddress")
						.append("orderStatus","$orderStatus")
						.append("totalCost","$totalCost")
...
//								.append("then", "$$KEEP").append("else", "$$PRUNE")))),
				...
				if(orderDiagnosticTests != null && !orderDiagnosticTests.isEmpty()) {
					...
					if(response.getDiagnosticTests() != null && !response.getDiagnosticTests().isEmpty())response.setTestsPackages(null);
				}
				else {
