...
public class UserFavouriteServicesimpl implements UserFavouriteService {
	...
	@Override
	public Boolean addRemoveFavourites(String userId, String resourceId, String resourceType, String locationId,
			Boolean discarded) {
		Boolean response = false;
		try {
			ObjectId userObjectId = new ObjectId(userId), resourceObjectId = new ObjectId(resourceId),
					locationObjectId = !DPDoctorUtils.anyStringEmpty(locationId) ? new ObjectId(locationId) : null;
			UserResourceFavouriteCollection resourceFavouriteCollection = userResourceFavouriteRepository
					.findByUserIdAndResourceIdAndResourceTypeAndLocationId(userObjectId, resourceObjectId,
							resourceType.toUpperCase(), locationObjectId);
			if (discarded) {
				if (resourceFavouriteCollection == null)
					throw new BusinessException(ServiceError.Unknown,
							resourceType + " does not exist in your favourites list.");
				else {
		...
		try {
			...
			if (size > 0) {
				aggregation = Aggregation.newAggregation(
						Aggregation.match(new Criteria("userId").is(new ObjectId(userId)).and("resourceType")
								.is(Resource.DOCTOR.name()).and("discarded").is(false)),
						...
						Aggregation.lookup(
								"doctor_clinic_profile_cl", "resourceId", "doctorId", "clinicProfileCollection"),
						...
						new CustomAggregationOperation(new Document("$redact",
								new BasicDBObject("$cond", new BasicDBObject("if",
										new BasicDBObject("$eq",
												Arrays.asList("$clinicProfileCollection.locationId", "$locationId")))
			...
			List<FavouriteLookupResponse> doctors = mongoTemplate
					.aggregate(aggregation, UserResourceFavouriteCollection.class, FavouriteLookupResponse.class)
					.getMappedResults();
	...
	public List<ESUserLocaleDocument> getFavouritePharmacies(long page, int size, String userId) {
		...
		try {
			...
			if (size > 0) {
				aggregation = Aggregation.newAggregation(
						Aggregation.match(new Criteria("userId").is(new ObjectId(userId)).and("resourceType")
								.is(Resource.PHARMACY.name()).and("discarded").is(false)),
						Aggregation.lookup("locale_cl", "resourceId", "_id", "pharmacy"),
						Aggregation.unwind("pharmacy"),
						Aggregation.lookup("user_cl", "$pharmacy.contactNumber", "mobileNumber", "user"),
			...
			List<FavouriteLookupResponse> favouriteLookupResponses = mongoTemplate
					.aggregate(aggregation, UserResourceFavouriteCollection.class, FavouriteLookupResponse.class)
					.getMappedResults();
	...
	public List<LabResponse> getFavouriteLabs(long page, int size, String userId) {
		...
		try {
			...
			if (size > 0) {
				aggregation = Aggregation.newAggregation(
						Aggregation.match(new Criteria("userId").is(new ObjectId(userId)).and("resourceType")
								.is(Resource.LAB.name()).and("discarded").is(false)),
				...
				for (FavouriteLookupResponse favouriteLookupResponse : favouriteLookupResponses) {
					...
					if (favouriteLookupResponse.getLab().getImages() != null)
						for (ClinicImage clinicImage : favouriteLookupResponse.getLab().getImages()) {
							...
							if (!DPDoctorUtils.anyStringEmpty(imgURL))
								images.add(imgURL);
						}
					...
					String address = (!DPDoctorUtils.anyStringEmpty(favouriteLookupResponse.getLab().getStreetAddress())
							...
							+ (!DPDoctorUtils.anyStringEmpty(favouriteLookupResponse.getLab().getLandmarkDetails())
									...
									: "")
							+ (!DPDoctorUtils.anyStringEmpty(favouriteLookupResponse.getLab().getLocality())
									? favouriteLookupResponse.getLab().getLocality() + ", "
									: "")
							+ (!DPDoctorUtils.anyStringEmpty(favouriteLookupResponse.getLab().getCity())
									? favouriteLookupResponse.getLab().getCity() + ", "
									: "")
							+ (!DPDoctorUtils.anyStringEmpty(favouriteLookupResponse.getLab().getState())
									? favouriteLookupResponse.getLab().getState() + ", "
									: "")
							+ (!DPDoctorUtils.anyStringEmpty(favouriteLookupResponse.getLab().getCountry())
									? favouriteLookupResponse.getLab().getCountry() + ", "
									: "")
							+ (!DPDoctorUtils.anyStringEmpty(favouriteLookupResponse.getLab().getPostalCode())
									? favouriteLookupResponse.getLab().getPostalCode()
									: "");
