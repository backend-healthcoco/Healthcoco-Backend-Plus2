...
public class EmailTrackServiceImpl implements EmailTackService {
	...
	@Override
	@Transactional
	public List<EmailTrack> getEmailDetails(String patientId, String doctorId, String locationId, String hospitalId,
			long page, int size) {
		...
		try {
			...
			Criteria criteria = new Criteria("type").in("APPOINTMENT", ComponentType.PRESCRIPTIONS.getType(),
					ComponentType.VISITS.getType(), ComponentType.CLINICAL_NOTES.getType(),
			...
			if (doctorObjectId == null) {
				...
				if (DPDoctorUtils.anyStringEmpty(locationObjectId, hospitalObjectId))
					criteria.and("doctorId").is(doctorObjectId).and("patientId").is(patientObjectId);
				else
					criteria.and("doctorId").is(doctorObjectId).and("locationId").is(locationObjectId).and("hospitalId")
							.is(hospitalObjectId).and("patientId").is(patientObjectId);
			}
			Aggregation aggregation = null;
			if (size > 0) {
				aggregation = Aggregation.newAggregation(Aggregation.match(criteria),
						Aggregation.sort(new Sort(Sort.Direction.DESC, "sentTime")), Aggregation.skip((page) * size),
						Aggregation.limit(size));
			} else {
				aggregation = Aggregation.newAggregation(Aggregation.match(criteria),
						Aggregation.sort(new Sort(Sort.Direction.DESC, "sentTime")));
			}
			AggregationResults<EmailTrack> aggregationResults = mongoTemplate.aggregate(aggregation,
					EmailTrackCollection.class, EmailTrack.class);
