...
//	@Override
	...
	@Override
	@Transactional
	public List<PatientTreatmentResponse> getPatientTreatmentsNEWCODE(int page, int size, String doctorId, String locationId,
			String hospitalId, String patientId, String updatedTime, Boolean isOTPVerified,String from,String to, Boolean discarded,
			Boolean inHistory, String status) {
		List<PatientTreatmentResponse> response = null;
		try {
			...
			if (size > 0)
				aggregation = Aggregation.newAggregation(Aggregation.match(criteria),
//						new CustomAggregationOperation(new Document("$unwind",
//								new BasicDBObject("path", "$_id").append("preserveNullAndEmptyArrays", true))),
...
//										true))),
						new CustomAggregationOperation(new Document("$unwind",
								new BasicDBObject("path", "$treatments")
...
//						patientTreatmentSecondGroupAggregationOperation(),
				...
				aggregation = Aggregation.newAggregation(Aggregation.match(criteria),
						...
						Aggregation.lookup("treatment_services_cl", "patientTreatment.treatments.treatmentServiceId",
								"_id", "treatmentService"),
						Aggregation.lookup("user_cl", "patientTreatment.treatments.doctorId", "_id", "treatmentDoctor"),
	...
	private AggregationOperation patientTreatmentFirstProjectAggregationOperation() {
		return new CustomAggregationOperation(new Document("$project",
				new BasicDBObject("_id", "$_id").append("uniqueEmrId", "$uniqueEmrId").append("patientId", "$patientId")
						...
						.append("visitId", "$patientVisit._id").append("appointmentId", "$appointmentId")
						...
						.append("time", "$time")
//						.append("patientTreatmentfromDate", "$treatments.fromDate")
						.append("patientId", "$patientId")
						.append("totalCost", "$totalCost")
						.append("totalDiscount", "$totalDiscount")
						.append("grandTotal", "$grandTotal")
						
						.append("treatments.createdTime", "$treatments.createdTime")
						.append("treatments.createdBy", "$treatments.createdBy")
						.append("treatments.updatedTime", "$treatments.updatedTime")
						.append("treatments.doctorName", "$treatmentDoctor.firstName")
						.append("treatments.doctorId", "$treatments.doctorId")
						.append("treatments.treatmentService", "$treatmentService")
						.append("treatments.treatmentServiceId", "$treatments.treatmentServiceId")
						.append("treatments.status", "$treatments.status")
						.append("treatments.treatmentDate", "$treatments.treatmentDate")
...
//				.append("totalCost", new BasicDBObject("$first", "$totalCost"))
				.append("totalDiscount", new BasicDBObject("$first", "$totalDiscount"))
				.append("grandTotal", new BasicDBObject("$first", "$grandTotal"))
//				.append("apappointmentId", new BasicDBObject("$first", "$appointmentId"))
//				.append("visitId", new BasicDBObject("$first", "$visitId"))
				.append("createdTime", new BasicDBObject("$first", "$createdTime"))
//				.append("createdBy", new BasicDBObject("$first", "$createdBy"))
				.append("updatedTime", new BasicDBObject("$first", "$updatedTime"))
	...
	private AggregationOperation patientTreatmentSecondProjectAggregationOperation() {
		return new CustomAggregationOperation(new Document("$project",
				new BasicDBObject("_id", "$_id").append("uniqueEmrId", "$uniqueEmrId").append("patientId", "$patientId")
						...
						.append("appointmentRequest", "$appointmentRequest").append("createdTime", "$createdTime")
						.append("updatedTime", "$updatedTime").append("createdBy", "$createdBy")
//						.append("uniqueEmrId", "$uniqueEmrId")
