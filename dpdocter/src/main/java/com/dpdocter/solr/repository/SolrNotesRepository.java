...
public interface SolrNotesRepository extends SolrCrudRepository<SolrNotesDocument, String> {
    ...
    @Query("(doctorId:*?0* AND updatedTime: {?1 TO *} AND ( discarded: ?2 OR discarded:false)  AND notes:*?3*) OR (doctorId:\"\" AND updatedTime: {?1 TO *} AND ( discarded: ?2 OR discarded:false)  AND notes:*?3*)")
    ...
    @Query("(doctorId:*?0* AND locationId:*?1* AND hospitalId:*?2* AND updatedTime: {?3 TO *} AND ( discarded: ?4 OR discarded:false) AND notes:*?5*) OR (doctorId:\"\" AND locationId:\"\" AND hospitalId: \"\" AND updatedTime: {?3 TO *} AND ( discarded: ?4 OR discarded:false) AND notes:*?5*)")
    public List<SolrNotesDocument> findCustomGlobalNotes(String doctorId, String locationId, String hospitalId, Date date, Boolean discarded,
    ...
    @Query("doctorId: \"\" AND updatedTime: {?0 TO *} AND ( discarded: ?1 OR discarded:false) AND notes:*?2*")
