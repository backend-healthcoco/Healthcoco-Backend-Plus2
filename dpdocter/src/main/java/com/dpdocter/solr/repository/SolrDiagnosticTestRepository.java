...
public interface SolrDiagnosticTestRepository extends SolrCrudRepository<SolrDiagnosticTestDocument, String> {
	...
	List<SolrDiagnosticTestDocument> getCustomDiagnosticTests(String locationId, String hospitalId, Date date, boolean discarded, String searchTerm, Pageable pageable);
	...
	List<SolrDiagnosticTestDocument> getCustomGlobalDiagnosticTests(String locationId, String hospitalId, Date date, boolean discarded, String searchTerm, Pageable pageable);

	@Query("(locationId:*?0* AND hospitalId:*?1* AND updatedTime: {?2 TO *} AND ( discarded: ?3 OR discarded:false) AND testName:?4*) OR (((locationId: null OR locationId: \"\") AND (hospitalId: null OR hospitalId: \"\") AND updatedTime: {?2 TO *} AND ( discarded: ?3 OR discarded:false) AND testName: ?4*)")
	List<SolrDiagnosticTestDocument> getCustomGlobalDiagnosticTests(String locationId, String hospitalId, Date date, boolean discarded, String searchTerm, Sort sort);
