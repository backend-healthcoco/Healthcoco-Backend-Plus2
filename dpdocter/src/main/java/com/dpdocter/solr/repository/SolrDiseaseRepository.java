...
public interface SolrDiseaseRepository extends SolrCrudRepository<SolrDiseasesDocument, String> {
	...
	List<SolrDiseasesDocument> findCustomDiseases(String doctorId, String locationId, String hospitalId, Date date, Boolean discarded, String searchTerm, Pageable pageable);

	@Query("doctorId:*?0* AND locationId:*?1* AND hospitalId:*?2* AND updatedTime: {?3 TO *} AND ( discarded: ?4 OR discarded:false) AND disease:?5*")
	List<SolrDiseasesDocument> findCustomDiseases(String doctorId, String locationId, String hospitalId, Date date, Boolean discarded, String searchTerm, Sort sort);
	...
	List<SolrDiseasesDocument> findCustomGlobalDiseases(String doctorId, String locationId, String hospitalId, Date date, Boolean discarded, String searchTerm, Pageable pageable);

	@Query("(doctorId:*?0* AND locationId:*?1* AND hospitalId:*?2* AND updatedTime: {?3 TO *} AND ( discarded: ?4 OR discarded:false) AND disease:?5*) OR ( (doctorId: null OR doctorId: \"\") AND (locationId: null OR locationId: \"\") AND (hospitalId: null OR hospitalId: \"\") AND updatedTime: {?3 TO *} AND ( discarded: ?4 OR discarded:false) AND disease:?5*)")
	List<SolrDiseasesDocument> findCustomGlobalDiseases(String doctorId, String locationId, String hospitalId, Date date, Boolean discarded, String searchTerm, Sort sort);
