...
public interface SolrPatientRepository extends SolrCrudRepository<SolrPatientDocument, String>, PagingAndSortingRepository<SolrPatientDocument, String> {
    @Query("doctorId:*?0* AND locationId:*?1* AND hospitalId:*?2* AND (firstName:*?3* OR emailAddress:*?3* OR mobileNumber:*?3* OR PID:*?3*)")
    List<SolrPatientDocument> find(String doctorId, String locationId, String hospitalId, String searchTerm, Pageable pageRequest);
    ...
    // List<SolrPatientDocument> findByCity(String doctorId, String locationId,
    // String hospitalId, String searchValue);
    //
    // @Query("doctorId:*?0* AND locationId:*?1* AND hospitalId:*?2* AND locality:*?3*")
    // List<SolrPatientDocument> findByLocality(String doctorId, String
    // locationId, String hospitalId, String searchValue);
    //
    // @Query("doctorId:*?0* AND locationId:*?1* AND hospitalId:*?2* AND bloodGroup:*?3*")
    // List<SolrPatientDocument> findByBloodGroup(String doctorId, String
    // locationId, String hospitalId, String searchValue);
    //
    // @Query("doctorId:*?0* AND locationId:*?1* AND hospitalId:*?2* AND referredBy:*?3*")
    // List<SolrPatientDocument> findByReferredBy(String doctorId, String
    // locationId, String hospitalId, String searchValue);
    //
    // @Query("doctorId:*?0* AND locationId:*?1* AND hospitalId:*?2* AND profession:*?3*")
    // List<SolrPatientDocument> findByProfession(String doctorId, String
    // locationId, String hospitalId, String searchValue);
    //
    // @Query("doctorId:*?0* AND locationId:*?1* AND hospitalId:*?2* AND postalCode:*?3*")
    // List<SolrPatientDocument> findByPostalCode(String doctorId, String
    // locationId, String hospitalId, String searchValue);
    //
    // @Query("doctorId:*?0* AND locationId:*?1* AND hospitalId:*?2* AND gender:?3")
    // List<SolrPatientDocument> findByGender(String doctorId, String
    // locationId, String hospitalId, String searchValue);
    ...
    List<SolrPatientDocument> count(String doctorId, String locationId, String hospitalId, String searchTerm);
