...
public interface SolrReferenceRepository extends SolrCrudRepository<SolrReferenceDocument, String>{
	...
	List<SolrReferenceDocument> findCustom(String doctorId, String locationId, String hospitalId, Date date, Boolean discarded, String searchTerm, Pageable pageable);
	...
	List<SolrReferenceDocument> findCustomGlobal(String doctorId, String locationId, String hospitalId, Date date, Boolean discarded, String searchTerm, Pageable pageable);

	@Query("(doctorId:*?0* AND locationId:*?1* AND hospitalId:*?2* AND updatedTime: {?3 TO *} AND ( discarded: ?4 OR discarded:false) AND reference:?5*) OR ( (doctorId: null OR doctorId: \"\") AND (locationId: null OR locationId: \"\") AND (hospitalId: null OR hospitalId: \"\") AND updatedTime: {?3 TO *} AND ( discarded: ?4 OR discarded:false) AND reference:?5*)")
	List<SolrReferenceDocument> findCustomGlobal(String doctorId, String locationId, String hospitalId, Date date, Boolean discarded, String searchTerm, Sort sort);
