...
public class SubscriptionApi {
	...
	public Response<PackageDetailObject> getPackageDetailByPackageName(
	...
	public Response<Boolean> verifySignature(@RequestBody SubscriptionPaymentSignatureRequest request) {
		...
		if (DPDoctorUtils.anyStringEmpty(request.getDoctorId(), request.getOrderId(), request.getSignature(),
	...
	public Response<Subscription> getSubscriptionHistoryByDoctorId(@PathParam("doctorId") String doctorId,
		...
		if (count > 0)
			response.setDataList(
					subscriptionService.getSubscriptionHistory(doctorId, size, page, isDiscarded, searchTerm));
		response.setCount(count);
		return response;
